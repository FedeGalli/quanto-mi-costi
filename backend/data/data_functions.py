...
class DataManager:
    ...
    def get_price_trend(self, com: str):
        trend_df = self._get_prices_filtered(com, str(self.price_start_year))
        for i in range((self.price_current_year - self.price_start_year) * 2):
            trend_df = pl.concat(
                [
                    # Building the semester/year with variable i
                    self._get_prices_filtered(com, "0" + str(((i % 2) * 5) + 1) + "/" + str(self.price_start_year + int(i / 2) + 1)),
                    trend_df
                ], how="vertical"
            )
    ...
    def get_volume_trend_mq(self, com: str):
        trend_df = self._get_volume_filtered(com, str(self.volume_start_year))
        for i in range(self.volume_current_year - self.volume_start_year):
            trend_df = pl.concat([self._get_volume_filtered(com, str(self.volume_start_year + i + 1)), trend_df], how="vertical")
...
d = DataManager()
print(d.get_current_volume_mq("BRESCIA", 100))
print(d.get_price_trend("BRESCIA"))


# Make the tends to be more grupped.
# For volume only expose only the desired column MQ
# For price expose only the selected zone, type and state
